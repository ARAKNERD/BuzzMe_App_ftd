...
const UpdateTimeRangeHours=(props)=>{
    ...
    const handleUpdate = async(e) =>{
        ...
        if(startTime.length>0 || endTime.length>0){
            setLoading(true)
            const server_response = await ajaxStation.updateStationTimeRange(props.timeID, startTime, endTime);
            setLoading(false);
...
export default UpdateTimeRangeHours
