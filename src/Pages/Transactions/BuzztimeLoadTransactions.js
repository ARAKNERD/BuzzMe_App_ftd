...
function BuzztimeLoadTransactions() {
  const [buzztimeTransactions, setBuzztimeTransactions] = useState([]);
  ...
  useEffect(() => {
    if (searchTerm || startDate || endDate) {
      ...
      getBuzztimeTransactions(page, account);
    }
  }, [page, account]);
      ...
      <div class="heading-layout1 mg-b-5">
        ...
        {loading || loading2 ? (
          ...
          <table className="table display data-table text-nowrap">
            ...
            <tbody>
              {buzztimeTransactions.length > 0 ? (
                buzztimeTransactions.map((item, index) => (
                  <tr key={index}>
