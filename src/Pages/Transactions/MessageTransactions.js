...
function MessageTransactions() {
  ...
  const getSmsTransactions = async (currentPage) => {
    setLoading(true);
  ...
  const exportToPDF = () => {
    ...
    const data = smsTransactions.map(item => [
  ...
  useEffect(() => {
    if (searchTerm || startDate || endDate) {
      ...
      getSmsTransactions(page, account);
    }
  }, [page, account]);
          ...
          <table className="table display data-table text-nowrap">
            ...
            <tbody>
              {smsTransactions.length > 0 ? (
                smsTransactions.map((item, index) => (
                  <tr key={index}>
